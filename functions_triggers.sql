...
CREATE OR REPLACE FUNCTION decrease_number_of_remaining_trainings() RETURNS TRIGGER AS
$$
begin
    if (SELECT number_of_remaining_trainings FROM client WHERE client.id = new.client_id) = 0
    then
        raise exception 'No trainings left';
...
CREATE OR REPLACE TRIGGER reduce_remaining_trainings AFTER INSERT ON signing_up_for_trainings
    FOR EACH ROW EXECUTE FUNCTION decrease_number_of_remaining_trainings();

--Уменьшение количества свободных мест на тренировке
CREATE OR REPLACE FUNCTION decrease_number_of_free_places_in_group() RETURNS TRIGGER AS
$$
begin
    ...
    then
        ...
        raise exception 'There are no empty seats in the group %', new.group_number;
    end if;
    return new;
end;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE TRIGGER reduce_free_places_in_group AFTER INSERT ON signing_up_for_trainings
    FOR EACH ROW EXECUTE FUNCTION decrease_number_of_free_places_in_group();

--Увеличение количества оставшихся тренировок на абонементе
CREATE OR REPLACE FUNCTION increase_number_of_remaining_trainings() RETURNS TRIGGER AS
$$
begin
...
CREATE OR REPLACE TRIGGER increase_remaining_trainings AFTER DELETE ON signing_up_for_trainings
    FOR EACH ROW EXECUTE FUNCTION increase_number_of_remaining_trainings();


--Увеличение колиества оставшихся мест на тренировке
CREATE OR REPLACE FUNCTION increase_number_of_free_places_in_group() RETURNS TRIGGER AS
$$
begin
    UPDATE training_schedule SET number_of_free_places = number_of_free_places + 1
...
CREATE OR REPLACE TRIGGER increase_free_places_in_group AFTER DELETE ON signing_up_for_trainings
    FOR EACH ROW EXECUTE FUNCTION increase_number_of_free_places_in_group();
...
CREATE OR REPLACE TRIGGER add_trainings_to_client AFTER INSERT ON purchase_of_subscription
